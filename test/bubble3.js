d3.json("data.json", function(error, data) {
	...
	var radiusM = (windowW/6)*2-50;
	var radiusL = (windowW/6)*3-100;
	...
	function go(){
		...
		if (windowSize < 640) {
			windowW = (windowSize/10)*8; 

			circleX = (windowW/2);
			circleY = (windowW/2);
			radiusS = (windowW/10)*2;
			radiusM = (windowW/10)*3;
	...
	var fociRH= {
		"Reg002_High002": { x: 600, y: 600, color: "#78E299", kor: "TK, 경북고"},
		"etc":  { x: 1300, y: 900, color: "#cccccc" , kor:"기타" }, 
	...
	var fociRU = {
		"Reg001_Univ001": { x: 600, y: 600, color: "#78E299", kor: "PK, 서울대" }, 
		"etc":  { x: 1300, y: 900, color: "#cccccc", kor:"기타" }, 
 	...
 	var fociHU = {
 		"High001_Univ001": { x: 600, y: 600, color: "#78E299", kor: "경기고, 서울대" }, 
		"etc":  { x: 1300, y: 900, color: "#cccccc", kor:"기타" }, 
	...
	var fociRHU = {
		"Reg002_High002_Univ001": { x: 600, y: 600, color: "#78E299", kor: "TK, 경북고, 서울대" },
	...
	var foci = {
		"toppos": { x: circleX, y: circleX, color: "#cccccc" }
	};
	...
	function circleDrawB(xx,yy,r) {
		//xx=원의중심x좌표, yy=원의중심y좌표, rr=원의반지름  
		var drawB = (selectValue - selectA);

		console.log(drawB + ' : ' + selectValue);
	...
	// 지역 + 고등학교 + 대학교 
  	function networkRHU() { 
		...
		for (var t=0; t<datalength; t++) {
			foci_index= 1;
			for (var l=0; l<selectValue; l++) {  
				var listRHU = d3.keys(foci)[l];
 
				if (data[t].RHU === listRHU) { 
					foci_index = l;  
				} 
			};  
			random_index= t; 
	...
	function tick(e) {  
		...
		.on("click",function(d,i) {
			...
			console.log('index : ' + i + '좌표 X : ' + d.x + ' Y : ' + d.y);
		}); 
	} // tick() 끝
	...
	function gravity(alpha) {  
		return function(d,i) {   
				...
				if (foci[d.choice].x == etcValue) {  
					var eachPie = (pie*2)/ (345-selectValue);

					var test = i * eachPie;

					d.x = etcValue + ( (Math.cos(test)) * radiusL ); //x = 반지름 * sin쎄타
					d.y = etcValue - ( (Math.sin(test)) *  radiusL ); //y = 반지름 * cos쎄타   
				}
		};
	} // gravity 끝
