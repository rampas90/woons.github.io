...
d3.json("data.json", function(error, data) {
	...
	var foci = {
		"toppos": { x: 100, y: 100, color: "#cccccc" }
	};
	...
	function networkRH() { 
		...
		circleDrawA(600,600, 200); 
		circleDrawB(600,600, 350);
	...
	function gravity(alpha) {  
		return function(d,i) {   
				...
				if (foci[d.choice].x == 601) {  
					var eachPie = (pie*2)/ (345-selectValue);

					var test = i * eachPie;

					d.x = 601 + ( (Math.cos(test)) * 500 ); //x = 반지름 * sin쎄타
					d.y = 601 - ( (Math.sin(test)) *  500 ); //y = 반지름 * cos쎄타   
				}
		};
	} // gravity 끝
